function update_total() { 
	...
	jQuery('.items-pajak-total').val(st_pajak.toFixed(2));
  ...
  jQuery(document).on('change click','.type_pajak', function () {
	 ...
	 if(type_pajak==='fixed'){
	 	...
	 	var subTotal = sbT + pajakPP;
	 	var sub_total = subTotal.toFixed(2);
	 	jQuery(this).closest('.item-row').find('.pajak-nilai-item').val(singlePajaks);
	 	jQuery(this).closest('.item-row').find('.sub-total-item').val(sub_total);
	 	update_total();
	 } else {
	 	var pajakPP = sbT / 100 * nilai_pajak;
...
update_total();
  ...
  $(document).ready(function(){	
  	...
  	jQuery("#aj_client_id").change(function(){
  		jQuery.get(base_url+"/get_projects_client/"+jQuery(this).val(), function(data, status){
