...
$(document).ready(function() {
    ...
    $('.edit-modal-data-variasi').on('show.bs.modal', function(event) {
        ...
        $.ajax({
            ...
            type: "GET",
            data: 'jd=1&is_ajax=1&mode=modal&data=variasi_project&variasi_id=' + variasi_id + "&mname=" + mname,
            success: function(response) {
                if (response) {
    ...
    $("#umb-form-variasi").submit(function(e) {
        e.preventDefault();
        var obj = $(this),
            action = obj.attr('name');
        $('.save').prop('disabled', true);
        $.ajax({
            type: "POST",
            url: e.target.action,
            data: obj.serialize() + "&is_ajax=1&add_type=variasi&form=" + action,
            cache: false,
            success: function(JSON) {
                if (JSON.error != '') {
                    toastr.error(JSON.error);
