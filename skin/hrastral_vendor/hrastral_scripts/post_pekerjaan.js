...
$('.edit-modal-data').on('show.bs.modal', function (event) {
	...
	$.ajax({
		url : base_url+"/read/",
		type: "GET",
		data: 'jd=1&is_ajax=1&mode=modal&data=pekerjaan&pekerjaan_id='+pekerjaan_id,
...
$("#umb-form").submit(function(e){
	...
	$.ajax({
		type: "POST",
		url: e.target.action,
		data: obj.serialize()+"&is_ajax=1&add_type=pekerjaan&form="+action,
		cache: false,
		success: function (JSON) {
			if (JSON.error != '') {
				toastr.error(JSON.error);
