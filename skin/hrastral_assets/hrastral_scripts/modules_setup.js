...
jQuery(".switch").change(function(){
	...
	if($('#m-perjalanan').is(':checked')){
		var mperjalanan = $("#m-perjalanan").val();
	} else {
		var mperjalanan = '';
	}
	...
	$.ajax({
		type: "GET",  url: site_url+"settings/modules_info/?is_ajax=2&type=modules_info&form=2&mrecruitment="+mrecruitment+"&mperjalanan="+mperjalanan+"&mfiles="+mfiles+"&mlanguage="+mlanguage+"&morgchart="+morgchart+"&mevents="+mevents+"&chatbox="+chatbox+'&is_sub_departments='+is_sub_departments+'&module_payroll='+module_payroll+'&module_performance='+module_performance,
		//data: order,
