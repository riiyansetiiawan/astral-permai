...
$(document).ready(function(){
	...
	// chatbox > department
	$('.chatbox-group-department').on('show.bs.modal', function (event) {
		var button = $(event.relatedTarget);
		var from_id = button.data('from-id');
		var chat_group_department_id = button.data('chat_group_department_id');
		var modal = $(this);
		$.ajax({
			url : base_url+"/read_department_chat/",
			type: "GET",
	...
	$('.chatbox-location-group').on('show.bs.modal', function (event) {
		var button = $(event.relatedTarget);
		var from_id = button.data('from-id');
		var group_location_chat_id = button.data('group_location_chat_id');
		var modal = $(this);
		$.ajax({
			url : base_url+"/read_location_chat/",
			type: "GET",
			data: 'jd=1&is_ajax=1&mode=modal&data=chat&from_id='+from_id+'&group_location_chat_id='+group_location_chat_id,
			success: function (response) {
				if(response) {
