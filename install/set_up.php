...
if(!empty($_POST) && $_POST['is_ajax']=='1' && $_POST['type']=='set_up'){
	...
	$sql = "UPDATE umb_system_setting SET application_name='$application_name',system_timezone='$system_timezone' WHERE setting_id=1";

	if ($conn->query($sql) === TRUE) {
		$options = array('cost' => 12);
		$password_hash = password_hash($password, PASSWORD_BCRYPT, $options);
		$user_sql = "UPDATE umb_karyawans SET first_name='$first_name',last_name='$last_name',email='$email',username='$username',password='$password_hash' WHERE user_id=1";
		$conn->query($user_sql);
		$_SESSION['admin_username'] = $username;
	  	$Return['result'] = "You have successfully installed the hrastral - Ultimate HRM.";
	} else {
