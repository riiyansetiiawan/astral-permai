...
class Project_model extends CI_Model {
	...
	public function get_projects() {
		return $this->db->get("umb_projects");
	}
	...
	public function get_all_project_timelogs() {
		$sql = "SELECT * FROM umb_projects_timelogs";
		$query = $this->db->query($sql);
		return $query;
	}

	public function get_all_project_karyawan_timelogs($user_id) {
		$sql = "SELECT * FROM umb_projects_timelogs where karyawan_id = ?";
		$binds = array($user_id);
		$query = $this->db->query($sql, $binds);
		return $query;
	}
	...
	public function add($data){
		$this->db->insert('umb_projects', $data);
	...
	public function add_project_timelog($data){
		$this->db->insert('umb_projects_timelogs', $data);
		if ($this->db->affected_rows() > 0) {
			return true;
		} else {
			return false;
	...
	public function read_info_timelog($id) {
		...
		$sql = 'SELECT * FROM umb_projects_timelogs WHERE timelogs_id = ?';
		$binds = array($id);
		$query = $this->db->query($sql, $binds);
	...
	public function update_record_project_timelog($data, $id){
		...
		if( $this->db->update('umb_projects_timelogs',$data)) {
			return true;
		} else {
			return false;
		}		
	...
	public function delete_record($id){
		...
		$this->db->delete('umb_projects');
		
	}

	public function delete_record_timelog($id){
		$this->db->where('timelogs_id', $id);
		$this->db->delete('umb_projects_timelogs');
		
	}
	
	public function delete_record_bug($id){
		$this->db->where('bug_id', $id);
		$this->db->delete('umb_projects_bugs');
	...
	public function get_all_projects(){
		$query = $this->db->query("SELECT * from umb_projects");
		return $query->result();
	}
	
	public function add_new_attachment($data){
		$this->db->insert('umb_attachment_projects', $data);
		if ($this->db->affected_rows() > 0) {
			return true;
		} else {
			return false;
	...
	public function delete_record_attachment($id){
		...
		$this->db->delete('umb_attachment_projects');
		
	}
	
	public function get_diskusi($id) {
		
		$sql = 'SELECT * FROM umb_diskusi_project WHERE project_id = ? order by diskusi_id desc';
		$binds = array($id);
		$query = $this->db->query($sql, $binds);
	...
	public function add_diskusi($data){
		$this->db->insert('umb_diskusi_project', $data);
		if ($this->db->affected_rows() > 0) {
			return true;
		} else {
			return false;
	...
	public function update_record($data, $id){
		$this->db->where('project_id', $id);
		if( $this->db->update('umb_projects',$data)) {
			return true;
		} else {
			return false;
		}		
	...
	public function get_project_perusahaans($perusahaan_id) {
