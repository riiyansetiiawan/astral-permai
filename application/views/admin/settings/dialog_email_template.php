...
if(isset($_GET['jd']) && isset($_GET['template_id']) && $_GET['data']=='email_template'){
	...
	<div class="modal-body">
		<p class="text-center text-big mb-4"><strong><?php echo $this->lang->line('umb_edit_email_template');?></strong></p>
		<div class="row">
			<div class="col-md-12">
				<div class="form-group">
					<label for="name"><?php echo $this->lang->line('umb_template_name');?></label>
			...
			<div class="col-md-5">
				<div class="form-group">
					...
					<select class="form-control" name="status" data-plugin="select_hrm" data-placeholder="<?php echo $this->lang->line('dashboard_umb_status');?>">
						<option value=""></option>
						<option value="1" <?php if($status==1):?> selected="selected"<?php endif;?>><?php echo $this->lang->line('umb_karyawans_active');?></option>
						<option value="0" <?php if($status==0):?> selected="selected"<?php endif;?>><?php echo $this->lang->line('umb_karyawans_inactive');?></option>
					</select>
				</div>
			</div>
		</div>
		<div class="row">
			<div class="col-md-12">
				<div class="form-group">
					<label for="message"><?php echo $this->lang->line('umb_message');?></label>
					<textarea class="form-control" placeholder="<?php echo $this->lang->line('umb_message');?>" name="message" id="summernote">
					...
					$.ajax({
						...
						success: function (JSON) {
							if (JSON.error != '') {
								...
								var umb_email_table = $('#umb_email_table').dataTable({
									"bDestroy": true,
									"ajax": {
										url : "<?php echo site_url("admin/settings/list_email_template") ?>",
										type : 'GET'
									},
									"iDisplayLength": 20,
								...
								umb_email_table.api().ajax.reload(function(){ 
