...
if(isset($_GET['jd']) && isset($_GET['client_id']) && $_GET['data']=='lead'){
  ...
  <div class="modal-header">
    ...
    <h4 class="modal-title" id="edit-modal-data">
      ...
      <?php echo $this->lang->line('umb_project_edit_lead');?>
    </h4>
  </div>
          ...
          success: function(JSON) {
            if (JSON.error != '') {
              ...
              var umb_table = $('#umb_table').dataTable({
                "bDestroy": true,
                "ajax": {
                  url : "<?php echo site_url("admin/leads/list_leads") ?>",
                  type : 'GET'
                },
                dom: 'lBfrtip',
              ...
              $('input[name="csrf_hrastral"]').val(JSON.csrf_hash);
              $('.edit-modal-data-timelog').modal('toggle');
              $('.save').prop('disabled', false);
              Ladda.stopAll();
            }
          },
...
<?php } else if(isset($_GET['jd']) && $_GET['data']=='view_lead' && isset($_GET['client_id']) ){
  ...
  <form class="m-b-1">
    <div class="modal-body">
      <p class="text-center text-big mb-4"><strong><?php echo $this->lang->line('umb_project_view_lead');?></strong></p>
  ...
  <?php } else if(isset($_GET['jd']) && isset($_GET['client_id']) && $_GET['data']=='change_to_client' && $_GET['type']=='lead'){
    ...
    <div class="modal-header"> 
      ...
      <strong class="modal-title"><?php echo $this->lang->line('umb_change_lead_confirm');?></strong>
    </div>
    <div class="alert alert-danger"> 
      <strong><?php echo $this->lang->line('umb_d_not_lead_restored');?></strong> 
    </div>
    <div class="modal-footer">
      <?php $attributes = array('name' => 'change_lead', 'id' => 'change_lead', 'autocomplete' => 'off', 'role'=>'form');?>
      <?php $hidden = array('_method' => 'DELETE', '_token_del_file' => 'hrastral', 'token_type' => 'hrastral');?>
      <?php echo form_open(site_url('admin/leads/change_to_client/'.$client_id), $attributes, $hidden);?> <?php echo form_button(array('data-dismiss' => 'modal', 'type' => 'button', 'class' => 'btn btn-secondary', 'content' => '<i class="fas fa-check-square"></i> '.$this->lang->line('umb_close'))); ?> <?php echo form_button(array('name' => 'hrastral_form', 'type' => 'submit', 'class' => $this->Umb_model->form_button_class(), 'content' => '<i class="fas fa-check-square"></i> '.$this->lang->line('umb_lead_confirm_change_btn'))); ?> <?php echo form_close(); ?> 
    </div>
    <?php echo form_close(); ?> 
    <script type="text/javascript">
      $(document).ready(function(){
  ...
  <?php } else if(isset($_GET['jd']) && isset($_GET['leads_followup_id']) && $_GET['data']=='leads_followup'){
    ...
    <?php $attributes = array('name' => 'edit_info_followup', 'id' => 'edit_info_followup', 'autocomplete' => 'off', 'class'=>'m-b-1');?>
    ...
    <div class="modal-body">
      <div class="row">
        <div class="col-md-12">
          <div class="form-group">
            <label for="umb_lead_next_followup"><?php echo $this->lang->line('umb_lead_next_followup');?></label>
            <input class="form-control followup_date" placeholder="<?php echo $this->lang->line('umb_lead_next_followup');?>" name="next_followup" type="text" value="<?php echo $next_followup;?>">
          </div>
        </div>
        <div class="col-sm-12">
          <div class="form-group">
            <label for="umb_description"><?php echo $this->lang->line('umb_description');?></label>
            <input class="form-control" placeholder="<?php echo $this->lang->line('umb_description');?>" name="description" type="text" value="<?php echo $description;?>">
          </div>
        </div>
      </div>
    </div>
    ...
    <script type="text/javascript">
      $(document).ready(function(){
        ...
        jQuery("#edit_info_followup").submit(function(e){
          e.preventDefault();
          ...
          jQuery.ajax({
            type: "POST",
            url: e.target.action,
            data: obj.serialize()+"&is_ajax=16&data=edit_info_followup&type=edit_info_followup&form="+action,
            cache: false,
            success: function (JSON) {
              if (JSON.error != '') {
                toastr.error(JSON.error);
                ...
                var umb_table = $('#umb_table').dataTable({
                  "bDestroy": true,
                  "ajax": {
                    url : "<?php echo site_url("admin/leads/list_leads_followup/").$lead_id;?>",
                    type : 'GET'
                  },
                  "fnDrawCallback": function(settings){
                    $('[data-toggle="tooltip"]').tooltip();          
