...
if(isset($_GET['jd']) && isset($_GET['transfer_id']) && $_GET['data']=='transfer'){
  ...
  <?php $hidden = array('_method' => 'EDIT', '_token' => $transfer_id, 'ext_name' => $karyawan_id);?>
        ...
        <div class="row">
          <div class="col-md-12">
            <div class="form-group">
              ...
              <input class="form-control edate" placeholder="<?php echo $this->lang->line('umb_tanggal_transfer');?>" readonly name="tanggal_transfer" type="text" value="<?php echo $tanggal_transfer;?>">
            </div>
          </div>
        </div>
        <div class="row">
          <div class="col-md-6">
            <div class="form-group" id="department_ajx">
              <?php $cresult = $this->Perusahaan_model->ajax_perusahaan_info_departments($perusahaan_id);?>
              ...
              <select class="select2" data-plugin="select_hrm" data-placeholder="<?php echo $this->lang->line('umb_select_department');?>" name="transfer_department">
                ...
                <?php foreach($cresult as $department) {?>
                  <option value="<?php echo $department->department_id?>" <?php if($department->department_id==$transfer_department):?> selected="selected"<?php endif;?>><?php echo $department->nama_department;?></option>
                <?php } ?>
              </select>
            </div>
          </div>
          <div class="col-md-6">
            <div class="form-group" id="ajx_location">
              <?php $lresult = $this->Department_model->ajax_informasi_location($perusahaan_id);?>
              ...
              <select class="select2" data-plugin="select_hrm" data-placeholder="<?php echo $this->lang->line('umb_transfer_select_location');?>" name="transfer_location">
                ...
                <?php foreach($lresult as $location) {?>
                  <option value="<?php echo $location->location_id?>" <?php if($location->location_id==$transfer_location):?> selected="selected"<?php endif;?>><?php echo $location->nama_location;?></option>
                <?php } ?>
              </select>
            </div>
          </div>
        ...
        success: function (JSON) {
         if (JSON.error != '') {
          ...
          Ladda.stopAll();
              ...
              "bDestroy": true,
              "ajax": {
               url : "<?php echo site_url("admin/transfers/list_transfer") ?>",
               type : 'GET'
             },
             dom: 'lBfrtip',
...
<?php } else if(isset($_GET['jd']) && isset($_GET['transfer_id']) && $_GET['data']=='view_transfer'){
  ...
  <form class="m-b-1">
    <div class="modal-body">
      <p class="text-center text-big mb-4"><strong><?php echo $this->lang->line('umb_view_transfer');?></strong></p>
      <table class="footable-details table table-striped table-hover toggle-circle">
        <tbody>
          <tr>
            ...
            <tr>
              <th><?php echo $this->lang->line('umb_karyawan_transfer');?></th>
              ...
              <tr>
                <th><?php echo $this->lang->line('umb_tanggal_transfer');?></th>
                <td style="display: table-cell;"><?php echo $this->Umb_model->set_date_format($tanggal_transfer);?></td>
              </tr>
              <tr>
                <th><?php echo $this->lang->line('umb_transfer_to_department');?></th>
