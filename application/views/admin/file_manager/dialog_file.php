...
<div class="modal-header">
  ...
  <h4 class="modal-title" id="edit-modal-data"><?php echo $this->lang->line('umb_edit_file_name');?></h4>
</div>
  ...
  <div class="row">
    <div class="col-md-12">
      <div class="form-group">
        <label for="file_name"><?php echo $this->lang->line('umb_new_file_name');?></label>
        <input class="form-control" placeholder="<?php echo $this->lang->line('umb_new_file_name');?>" name="file_name" type="text" value="<?php echo $file;?>">
      </div>
    </div>
  </div>
</div>
			...
			$.ajax({
				...
				if (JSON.error != '') {
					...
					var umb_table_files = $('#umb_table_files').dataTable({
						"bDestroy": true,
						"ajax": {
							url : "<?php echo site_url("admin/files/list_files") ?>/dId/"+$('#did').val(),
							type : 'GET'
						},
					...
					$('input[name="csrf_hrastral"]').val(JSON.csrf_hash);
					$('.modal_payroll_template').modal('toggle');
					$('.save').prop('disabled', false);
					Ladda.stopAll();
				}
			}
