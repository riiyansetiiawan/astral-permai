...
if(isset($_GET['jd']) && isset($_GET['libur_id']) && $_GET['data']=='view_libur'){
  ?>
  <form class="m-b-1">
    <div class="modal-body">
      <p class="text-center text-big mb-4"><strong><?php echo $this->lang->line('umb_view_libur');?></strong></p>
      <table class="footable-details table table-striped table-hover toggle-circle">
        <tbody>
          <tr>
            ...
            <tr>
              <th><?php echo $this->lang->line('umb_event_name');?></th>
            ...
            <tr>
              ...
              <td style="display: table-cell;"><?php if($is_publish=='1'): $status = $this->lang->line('umb_published');?>
            <?php endif; ?>
            <?php if($is_publish=='0'): $status = $this->lang->line('umb_unpublished');?>
            <?php endif; ?>
            <?php echo $status;?></td>
          </tr>
          <tr>
    ...
    <?php echo form_close(); ?>
  <?php } else if(isset($_GET['jd']) && isset($_GET['libur_id']) && $_GET['data']=='libur'){
    ?>
    <?php $session = $this->session->userdata('username');?>
    <?php $user_info = $this->Umb_model->read_user_info($session['user_id']);?>
    <div class="modal-header">
      <button type="button" class="close" data-dismiss="modal" aria-label="Close"> <span aria-hidden="true">×</span> </button>
      <h4 class="modal-title" id="edit-modal-data"><?php echo $this->lang->line('umb_edit_libur');?></h4>
    </div>
    ...
    <?php echo form_open('admin/timesheet/edit_libur/'.$libur_id, $attributes, $hidden);?>
    <div class="modal-body">
      <div class="row">
        <div class="col-md-6">
          <div class="row">
                ...
                <div class="form-group">
                  ...
                  <select class="form-control" name="perusahaan_id" id="aj_perusahaan" data-plugin="select_hrm" data-placeholder="<?php echo $this->lang->line('left_perusahaan');?>">
                    ...
                    <?php foreach($get_all_perusahaans as $perusahaan) {?>
                      <option value="<?php echo $perusahaan->perusahaan_id?>" <?php if($perusahaan->perusahaan_id==$perusahaan_id) { ?> selected <?php } ?>><?php echo $perusahaan->name?></option>
                    <?php } ?>
                  </select>
                </div>
              <?php } else {?>
          ...
          <div class="form-group">
            <label for="title"><?php echo $this->lang->line('umb_event_name');?></label>
            <input class="form-control" placeholder="<?php echo $this->lang->line('umb_event_name');?>" name="event_name" type="text" value="<?php echo $event_name;?>">
          </div>
          <div class="row">
            <div class="col-md-6">
              <div class="form-group">
        ...
        <div class="col-md-6">
          ...
          <div class="row">
            <div class="col-md-12">
              <div class="form-group">
                <label for="penunjukan" class="control-label"><?php echo $this->lang->line('dashboard_umb_status');?></label>
                <select name="is_publish" class="select2" data-plugin="select_hrm" data-placeholder="<?php echo $this->lang->line('umb_choose_status');?>">
                  <option value="1" <?php if($is_publish=='1') { ?> selected <?php } ?>><?php echo $this->lang->line('umb_published');?></option>
                  <option value="0" <?php if($is_publish=='0') { ?> selected <?php } ?>><?php echo $this->lang->line('umb_unpublished');?></option>
                </select>
              </div>
            </div>
          </div>
     ...
     success: function (JSON) {
      if (JSON.error != '') {
       ...
       var umb_table = $('#umb_table').dataTable({
        "bDestroy": true,
        "ajax": {
         url : "<?php echo site_url("admin/timesheet/list_liburan") ?>",
         type : 'GET'
       },
       dom: 'lBfrtip',
