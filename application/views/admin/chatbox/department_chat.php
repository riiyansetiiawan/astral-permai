...
if(isset($_GET['jd']) && isset($_GET['from_id']) && isset($_GET['chat_group_department_id']) && $_GET['data']=='chat'){
	...
	$department = $this->Department_model->read_informasi_department($chat_group_department_id);
	...
	<div class="modal-header">
		...
		<h6 class="modal-title" id="myModalLabel8">Chat With <?php echo $this->lang->line('left_department');?> (<?php echo $_nama_department;?>)</h6>
	</div>
	<div class="modal-body" >
		<div class="content-body">
			<section class="chat-app-window">
				<div class="chats chat-app-window1" id="chatbox_mdepartmentchatbox" style="overflow:auto; height:450px;">
					<?php foreach($this->Chat_model->get_department_messages($user[0]->department_id) as $msgs){
						if($session['user_id']!=$msgs->from_id){
							...
							<div class="chat-message-right mb-4">
								...
								<div class="flex-shrink-1 bg-lighter rounded py-2 px-3 ml-3">
									<div class="font-weight-semibold mb-1">Kamu</div>
									<?php echo $msgs->message_content;?>
								</div>
							</div>
							<?php
			...
			<section class="chat-app-form">
				<div class="box-footer">
					<?php $attributes = array('name' => 'send_chat', 'id' => 'umb-department-form', 'autocomplete' => 'off', 'class'=>'chat-app-input');?>
					<?php $hidden = array('_method' => 'EDIT', '_token' => $chat_group_department_id, 'ext_name' => $fid);?>
					<?php echo form_open('admin/chat/kirim_chat_department/', $attributes, $hidden);?>
					...
					<div class="input-group">
						<input type="text" name="message_content" class="form-control" id="content_message_dep" placeholder="Type your message">
				...
				<audio id="chatAudioSent">
					<source src="
					<?php echo base_url().'uploads/chat_sound/button_tiny.ogg';?>" type="audio/ogg">
					<source src="
					<?php echo base_url().'uploads/chat_sound/button_tiny.mp3';?>" type="audio/mpeg">
					<source src="
	...
	<script type="text/javascript">
		...
		function mrefreshMessages() {
			...
			if(rfromId!='' && rtoId!='') {
				$.ajax({
					url: "<?php echo site_url('admin/chat/refresh_chatbox_department');?>?from_id=<?php echo $fid;?>&department_id=<?php echo $chat_group_department_id;?>",
					type: 'GET',
					dataType: 'html',
					success: function(data) {
						...
						$('.chat-application').removeClass('pace-running');
						$('.pace').removeClass('pace-inactive');
						prev_chat = $("#chatbox_mdepartmentchatbox").html();
							// $('input[name="csrf_hrastral"]').val(data.csrf_hash);
							jQuery('#chatbox_mdepartmentchatbox').html(data);
							curr_chat = $("#chatbox_mdepartmentchatbox").html();
							...
							if(curr_chat != prev_chat) {
								...
								if(first_run===0) {
									...
									if(prev_chat!='<div class="chats" id="chatbox_mdepartmentchatbox"></div>' ) {
										$('#chatAudio')[0].play();
		...
		$(document).ready(function(){
			$("#umb-department-form").submit(function(e){
				var fd = new FormData(this);
				var text = $("#content_message_dep").val();
				if(text.length == 0){
					return false;
				}			
				...
				$.ajax({
					...
					success: function(JSON) {
						...
						var siteUrl = "<?php echo site_url('admin/chat/refresh_chatbox_department');?>?from_id=<?php echo $fid;?>&department_id=<?php echo $chat_group_department_id;?>";
						$.get(siteUrl, function(data, status){
							jQuery("#chatbox_mdepartmentchatbox").html(data);

							$("#content_message_dep").val("");
							$("#content_message_dep").focus();
