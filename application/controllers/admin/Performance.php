...
class Performance extends MY_Controller {
	...
	public function index() {
		...
		if($system[0]->performance_option == 'appraisal'):
			$data['title'] = $this->lang->line('left_performance_xappraisal').' | '.$this->Umb_model->site_title();
			...
			$data['path_url'] = 'performance_tujuans';
		endif;
		...
		if(in_array('42',$role_resources_ids)) {
			if(!empty($session)){ 
				$data['subview'] = $this->load->view("admin/performance/list_performance", $data, TRUE);
