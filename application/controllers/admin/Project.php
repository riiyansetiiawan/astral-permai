...
class Project extends MY_Controller {
	...
	public function index()
	{
		...
		if(in_array('44',$role_resources_ids)) {
			if(!empty($session)){ 
				$data['subview'] = $this->load->view("admin/project/list_project", $data, TRUE);
	...
	public function dashboard_projects() {
		...
		$data['title'] = $this->lang->line('hr_title_dashboard_project').' | '.$this->Umb_model->site_title();
		...
		$data['breadcrumbs'] = $this->lang->line('hr_title_dashboard_project');
		$data['path_url'] = 'dashboard_projects';
		$role_resources_ids = $this->Umb_model->user_role_resource();
		if(in_array('44',$role_resources_ids)) {
			if(!empty($session)){ 
				$data['subview'] = $this->load->view("admin/project/dashboard_project", $data, TRUE);
				$this->load->view('admin/layout/layout_main', $data); 
			} else {
				redirect('admin/');
			}
	...
	public function timelogs() {
		...
		if(in_array('44',$role_resources_ids)) {
			if(!empty($session)){ 
				$data['subview'] = $this->load->view("admin/project/list_project_timelogs", $data, TRUE);
				$this->load->view('admin/layout/layout_main', $data); 
			} else {
				redirect('admin/');
			}
	...
	public function calendar_projects() {
		...
		$data['title'] = $this->lang->line('umb_hr_calendar_projects');
		$data['breadcrumbs'] = $this->lang->line('umb_hr_calendar_projects');
		$data['all_tugass'] = $this->Timesheet_model->get_tugass();
		$data['all_projects'] = $this->Project_model->get_projects();
		$data['path_url'] = 'calendar_projects';
		$role_resources_ids = $this->Umb_model->user_role_resource();
		if(in_array('44',$role_resources_ids)) {
			$data['subview'] = $this->load->view("admin/project/calendar_projects", $data, TRUE);
			$this->load->view('admin/layout/layout_main', $data); 
		} else {
			redirect('admin/dashboard');
		}
	...
	public function calendar_tugass() {
		
		$session = $this->session->userdata('username');
		if(empty($session)){ 
			redirect('admin/');
		}
		$data['title'] = $this->lang->line('umb_calendar_tugass');
		$data['breadcrumbs'] = $this->lang->line('umb_calendar_tugass');
		$data['all_tugass'] = $this->Timesheet_model->get_tugass();
		$data['all_projects'] = $this->Project_model->get_projects();
		$data['path_url'] = 'calendar_projects';
		$role_resources_ids = $this->Umb_model->user_role_resource();
		if(in_array('45',$role_resources_ids)) {
			$data['subview'] = $this->load->view("admin/project/calendar_tugass", $data, TRUE);
			$this->load->view('admin/layout/layout_main', $data); 
		} else {
			redirect('admin/dashboard');
		}
	...
	public function scrum_board_tugass() {
		
		$session = $this->session->userdata('username');
		if(empty($session)){ 
			redirect('admin/');
		}
		$data['title'] = $this->lang->line('umb_tugass_sboard');
		$data['breadcrumbs'] = $this->lang->line('umb_tugass_sboard');
		$data['all_tugass'] = $this->Timesheet_model->get_tugass();
		$data['all_projects'] = $this->Project_model->get_projects();
		$data['path_url'] = 'scrum_board_tugass';
		$role_resources_ids = $this->Umb_model->user_role_resource();
		if(in_array('45',$role_resources_ids)) {
			$data['subview'] = $this->load->view("admin/project/scrum_board_tugas", $data, TRUE);
			$this->load->view('admin/layout/layout_main', $data); 
		} else {
			redirect('admin/dashboard');
		}
	...
	public function scrum_board_projects() {
		
		$session = $this->session->userdata('username');
		if(empty($session)){ 
			redirect('admin/');
		}
		$data['title'] = $this->lang->line('umb_projects_sboard');
		$data['breadcrumbs'] = $this->lang->line('umb_projects_sboard');
		$data['all_tugass'] = $this->Timesheet_model->get_tugass();
		$data['all_projects'] = $this->Project_model->get_projects();
		$data['path_url'] = 'scrum_board_projects';
		$role_resources_ids = $this->Umb_model->user_role_resource();
		if(in_array('44',$role_resources_ids)) {
			$data['subview'] = $this->load->view("admin/project/scrum_board_projects", $data, TRUE);
			$this->load->view('admin/layout/layout_main', $data); 
		} else {
			redirect('admin/dashboard');
		}
	...
	public function get_karyawans() {
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view("admin/project/get_karyawans", $data);
		} else {
			redirect('admin/');
		}
	...
	public function get_project_karyawans() {
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view("admin/project/get_project_karyawans", $data);
		} else {
			redirect('admin/');
		}
	...
	public function update_status_scrum_board_tugas() {

		$data['title'] = $this->Umb_model->site_title();
		
		$Return = array('result'=>'', 'error'=>'', 'csrf_hash'=>'');
		$Return['csrf_hash'] = $this->security->get_csrf_hash();		
		$tugas_id = $this->uri->segment(4);
		$status_tugas = $this->uri->segment(5);
		
		$data = array(
			'status_tugas' => $status_tugas,
		);
		$result = $this->Timesheet_model->update_record_tugas($data,$tugas_id);
		if ($result == TRUE) {
	...
	public function update_status_scrum_board_project() {
		...
		if ($result == TRUE) {
			$Return['result'] = $this->lang->line('umb_sukses_status_project');
		} else {
			$Return['error'] = $this->lang->line('umb_error_msg');
		}
		$this->output($Return);
	...
	public function detail() {
		...
		$this->Umb_model->update_notification_record($edata,$id,$session['user_id'],'projects');
		...
		$data = array(
			...
			'jam_anggaran' => $result[0]->jam_anggaran,
		...
		if(!empty($session)){ 
			$data['subview'] = $this->load->view("admin/project/details_project", $data, TRUE);
			$this->load->view('admin/layout/layout_main', $data); 
		} else {
			redirect('admin/');
		}
	...
	public function list_project() {
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view("admin/project/list_project", $data);
		} else {
			redirect('admin/');
		}
		...
		if($user_info[0]->user_role_id==1){
			...
			if(in_array('318',$role_resources_ids)) {
				$project = $this->Project_model->get_project_perusahaans($user_info[0]->perusahaan_id);
			} else {
		...
		foreach($project->result() as $r) {
			...
			if(in_array('317',$role_resources_ids)) {
				$delete = '<span data-toggle="tooltip" data-placement="top" data-state="danger" title="'.$this->lang->line('umb_delete').'"><button type="button" class="btn icon-btn btn-sm btn-outline-danger waves-effect waves-light delete" data-toggle="modal" data-target=".delete-modal" data-record-id="'. $r->project_id . '"><span class="fas fa-trash-restore"></span></button></span>';
			} else {
				$delete = '';
			}
			...
			$ringkasan_project = '<a href="'.site_url().'admin/project/detail/'.$r->project_id . '">'.$r->title.'</a><br><small>'.$this->lang->line('umb_project_client').': '.$name_client.'</small><br><small>'.$this->lang->line('umb_project_budget_hrs').': '.$r->jam_anggaran.'</small><br><small>'.$this->lang->line('umb_project_actual_hrs').': '.$new_time.'</small>';
			
			$tanggal_project = $this->lang->line('umb_start_date').': '.$psdate.'<br>'.$this->lang->line('umb_end_date').': '.$pedate;

			$progress_project = $pbar.$status;
			$no_project = '<a href="'.site_url().'admin/project/detail/'.$r->project_id . '">'.$r->no_project.'</a>';
			$combhr = $edit.$delete;
			$data[] = array(
				$combhr,
				$no_project,
	...
	public function read() {
		...
		$data = array(
			...
			'summary' => $result[0]->summary,
			'no_project' => $result[0]->no_project,
			'no_pembelian' => $result[0]->no_pembelian,
	...
	public function get_scrumboard_tugas() {
		$data['title'] = $this->Umb_model->site_title();
		$status_tugas = $this->input->get('status_tugas');
		//$result = $this->Project_model->read_informasi_project($id);
		$data = array(
			'status_tugas' => $status_tugas,
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view('admin/project/dialog_scrumboard_tugas', $data);
		} else {
			redirect('admin/');
		}
	}
	...
	public function add_scrum_board_tugas() {
	...
	public function add_project() {
		...
		if($this->input->post('add_type')=='project') {		
			...
			if($this->input->post('title')==='') {
				...
				$Return['error'] = $this->lang->line('umb_error_start_end_date');
			} else if($this->input->post('jam_anggaran')==='') {
				$Return['error'] = $this->lang->line('umb_project_budget_hrs_field_error');
			...
			$data = array(
				...
				'jam_anggaran' => $this->input->post('jam_anggaran'),
			...
			if ($result == TRUE) {
				...
				$row = $this->db->select("*")->limit(1)->order_by('project_id',"DESC")->get("umb_projects")->row();
				...
				if($setting[0]->enable_email_notification == 'yes') {
					...
					foreach($this->input->post('assigned_to') as $p_karyawan) {
						...
						$user_info = $this->Umb_model->read_user_info($p_karyawan);				

						$cinfo = $this->Umb_model->read_info_setting_perusahaan(1);

						$template = $this->Umb_model->read_email_template(3);
						
						$subject = $template[0]->subject.' - '.$cinfo[0]->nama_perusahaan;
						$logo = base_url().'uploads/logo/signin/'.$cinfo[0]->sign_in_logo;
						
						$p_date = $this->Umb_model->set_date_format($start_date);
						
						$message = '
						<div style="background:#f6f6f6;font-family:Verdana,Arial,Helvetica,sans-serif;font-size:12px;margin:0;padding:0;padding: 20px;">
						<img src="'.$logo.'" title="'.$cinfo[0]->nama_perusahaan.'"><br>'.str_replace(array("{var site_name}","{var name}","{var nama_project}","{var project_start_date}"),array($cinfo[0]->nama_perusahaan,'User',$this->input->post('title'),$p_date),html_entity_decode(stripslashes($template[0]->message))).'</div>';
	...
	public function list_diskusi() {
		...
		foreach($diskusi->result() as $r) {
			...
			if($r->attachment_file!='' && $r->attachment_file!='no_file'){
				$at_file = '<a data-toggle="tooltip" data-placement="top" data-state="primary" title="'.$this->lang->line('umb_download').'" href="'.site_url().'admin/download?type=project/diskusi&filename='.$r->attachment_file.'"> <i class="oi oi-cloud-download"></i> </a>';
			} else {
				$at_file = '';
			}
	...
	public function set_diskusi() {
		...
		if($this->input->post('add_type')=='set_diskusi') {		
			...
			$result = $this->Project_model->add_diskusi($data);
			if ($result == TRUE) {
	...
	public function list_bug() {
		...
		foreach($bug->result() as $r) {
			...
			if($r->attachment_file!='' && $r->attachment_file!='no_file'){
				$at_file = '<a data-toggle="tooltip" data-placement="top" data-state="primary" title="'.$this->lang->line('umb_download').'" href="'.site_url().'admin/download?type=project/bug&filename='.$r->attachment_file.'"> <i class="oi oi-cloud-download"></i> </a>';
			} else {
				$at_file = '';
			}
			...
			<div class="c-rating">
			<span data-toggle="tooltip" data-placement="top" data-state="primary" title="'.$this->lang->line('umb_update_status').'"><button type="button" class="btn icon-btn btn-sm btn-outline-secondary waves-effect waves-light" data-toggle="modal" data-target=".add-modal-data"  data-bug_id="'. $r->bug_id . '"><i class="fas fa-pencil-alt"></i></button></span><span data-toggle="tooltip" data-placement="top" data-state="danger" title="'.$this->lang->line('umb_delete').'"><a class="btn icon-btn btn-sm btn-outline-danger delete" href="#" data-toggle="modal" data-target=".delete-modal" data-record-id="'.$r->bug_id.'"><span class="fas fa-trash-restore"></span></a></span>
			</div>
			</div>';
	...
	public function set_bug() {
		...
		if($this->input->post('add_type')=='set_bug') {		
			...
			if($this->input->post('title')==='') {
				$Return['error'] = $this->lang->line('umb_error_project_bug_title');
			} 
			$title = $this->input->post('title');
			...
			if ($result == TRUE) {
				$Return['result'] = $this->lang->line('umb_sukses_project_bug_ditambahkan');
			} else {
				$Return['error'] = $this->lang->line('umb_error_msg');
			}
			$this->output($Return);
	...
	public function list_attachment() {
		...
		foreach($attachments->result() as $r) {
			...
			$data[] = array('<span data-toggle="tooltip" data-placement="top" data-state="primary" title="'.$this->lang->line('umb_download').'"><a href="'.site_url().'admin/download?type=project/files&filename='.$r->attachment_file.'"><button type="button" class="btn icon-btn btn-sm btn-outline-secondary waves-effect waves-light"><span class="oi oi-cloud-download"></span></button></a></span><span data-toggle="tooltip" data-placement="top" data-state="danger" title="'.$this->lang->line('umb_delete').'"><button type="button" class="btn icon-btn btn-sm btn-outline-danger waves-effect waves-light fidelete" data-toggle="modal" data-target=".delete-modal-file" data-record-id="'. $r->project_attachment_id . '"><span class="fas fa-trash-restore"></span></button></span>',
				$r->file_title,
				$r->file_description,
				$r->created_at
			);
	...
	public function kategoris_tugas() {
		$session = $this->session->userdata('username');
		if(empty($session)){ 
			redirect('admin/');
		}
		...
		$data['title'] = $this->lang->line('umb_kategoris_tugas').' | '.$this->Umb_model->site_title();
		$data['breadcrumbs'] = $this->lang->line('umb_kategoris_tugas');
		$data['path_url'] = 'kategoris_tugas';
		$data['all_perusahaans'] = $this->Umb_model->get_perusahaans();
		$role_resources_ids = $this->Umb_model->user_role_resource();
		if(in_array('45',$role_resources_ids)) {
			if(!empty($session)){
				$data['subview'] = $this->load->view("admin/project/kategoris_tugas", $data, TRUE);
				$this->load->view('admin/layout/layout_main', $data); 
			} else {
				redirect('admin/');
			}
	...
	public function list_kategoris_tugas() {

		$data['title'] = $this->Umb_model->site_title();
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view("admin/project/kategoris_tugas", $data);
		} else {
			redirect('admin/');
		}
		...
		$kategoris_tugas = $this->Project_model->get_kategoris_tugas();
		$role_resources_ids = $this->Umb_model->user_role_resource();
		$data = array();

		foreach($kategoris_tugas->result() as $r) {
			
			if(in_array('346',$role_resources_ids)) { 
				$edit = '<span data-toggle="tooltip" data-placement="top" data-state="primary" title="'.$this->lang->line('umb_edit').'"><button type="button" class="btn icon-btn btn-sm btn-outline-secondary waves-effect waves-light"  data-toggle="modal" data-target=".edit-modal-data"  data-kategori_tugas_id="'. $r->kategori_tugas_id . '"><span class="fas fa-pencil-alt"></span></button></span>';
			} else {
				$edit = '';
			}
			if(in_array('347',$role_resources_ids)) {
				$delete = '<span data-toggle="tooltip" data-placement="top" data-state="danger" title="'.$this->lang->line('umb_delete').'"><button type="button" class="btn icon-btn btn-sm btn-outline-danger waves-effect waves-light delete" data-toggle="modal" data-target=".delete-modal" data-record-id="'. $r->kategori_tugas_id . '"><span class="fas fa-trash-restore"></span></button></span>';
			} else {
				$delete = '';
			}
		...
		$output = array(
			"draw" => $draw,
			"recordsTotal" => $kategoris_tugas->num_rows(),
			"recordsFiltered" => $kategoris_tugas->num_rows(),
			"data" => $data
		);
		echo json_encode($output);
		exit();
	...
	public function list_timelogs() {
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view("admin/project/list_project_timelogs", $data);
		} else {
			redirect('admin/');
		}
		...
		if($user_info[0]->user_role_id == '1'){
			...
			$timelogs = $this->Project_model->get_all_project_karyawan_timelogs($session['user_id']);
		}
		$role_resources_ids = $this->Umb_model->user_role_resource();
		$data = array();
		...
		foreach($timelogs->result() as $r) {
			...
			if(!is_null($project)){
				$nama_project = '<a target="_blank" href="'.site_url('admin/project/detail/').$r->project_id.'">'.$project[0]->title.'</a>';
			} else {
				$nama_project = '--';
			}
			...
			$delete = '<span data-toggle="tooltip" data-placement="top" data-state="danger" title="'.$this->lang->line('umb_delete').'"><button type="button" class="btn icon-btn btn-sm btn-outline-danger waves-effect waves-light delete" data-toggle="modal" data-target=".delete-modal" data-record-id="'. $r->timelogs_id . '"><span class="fas fa-trash-restore"></span></button></span>';		
			if($user_info[0]->user_role_id == '1'){
				$combhr = $edit.$delete;
			} else {
				$combhr = $edit;
	...
	public function list_project_timelogs() {
		...
		foreach($timelogs->result() as $r) {
			...
			$delete = '<span data-toggle="tooltip" data-placement="top" data-state="danger" title="'.$this->lang->line('umb_delete').'"><button type="button" class="btn icon-btn btn-sm btn-outline-danger waves-effect waves-light delete-timelog" data-toggle="modal" data-target=".delete-modal-timelogs" data-record-id="'. $r->timelogs_id . '"><span class="fas fa-trash-restore"></span></button></span>';
		//	} else {
		//		$delete = '';
		//	}
			if($user_info[0]->user_role_id == '1'){
	...
	public function add_kategori_tugas() {
		
		if($this->input->post('add_type')=='kategori_tugas') {		
			
			$Return = array('result'=>'', 'error'=>'', 'csrf_hash'=>'');
			$Return['csrf_hash'] = $this->security->get_csrf_hash();
			
			if($this->input->post('nama_kategori')==='') {
				$Return['error'] = $this->lang->line('umb_field_kategori_tugas_error');
			}
			...
			$result = $this->Project_model->add_kategoris_tugas($data);
			if ($result == TRUE) {
				$Return['result'] = $this->lang->line('umb_field_kategori_tugas_ditambahkan_success');
			} else {
				$Return['error'] = $this->lang->line('umb_error_msg');
			}
			$this->output($Return);
	...
	public function update_kategori_tugas() {
		
		if($this->input->post('edit_type')=='kategori_tugas') {
			...
			$result = $this->Project_model->update_record_kategori_tugas($data,$id);		
			
			if ($result == TRUE) {
				$Return['result'] = $this->lang->line('umb_field_kategori_tugas_diperbarui_success');
			} else {
				$Return['error'] = $this->lang->line('umb_error_msg');
			}
			$this->output($Return);
	...
	public function add_project_timelog() {
		...
		if($this->input->post('add_type')=='timelog') {		
			...
			$project = $this->Project_model->read_informasi_project($this->input->post('project_id'));	
			if(!is_null($project)){
	...
	public function update_project_timelog() {
		...
		if($this->input->post('edit_type')=='timelog_record') {
			...
			$result = $this->Project_model->update_record_project_timelog($data,$id);		
	...
	public function read_record_timelog() {
		...
		$result = $this->Project_model->read_info_timelog($id);
		$data = array(
	...
	public function read_record_project_timelog() {
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view('admin/project/dialog_record_project_timelogs', $data);
		} else {
			redirect('admin/');
		}
	}
	public function read_kategori_tugas()
	{
		$data['title'] = $this->Umb_model->site_title();
		$id = $this->input->get('kategori_tugas_id');
		$result = $this->Project_model->read_informasi_kategori_tugas($id);
		$data = array(
			'kategori_tugas_id' => $result[0]->kategori_tugas_id,
			'nama_kategori' => $result[0]->nama_kategori
		);
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view('admin/project/dialog_kategoris_tugas', $data);
		} else {
			redirect('admin/');
		}
	}

	public function delete_kategori_tugas() {
		
		$Return = array('result'=>'', 'error'=>'', 'csrf_hash'=>'');
		$id = $this->uri->segment(4);
		$Return['csrf_hash'] = $this->security->get_csrf_hash();
		$result = $this->Project_model->delete_record_kategori_tugas($id);
		if(isset($id)) {
			$Return['result'] = $this->lang->line('umb_field_kategori_tugas_dihapus_success');
		} else {
			$Return['error'] = $this->lang->line('umb_error_msg');
		}
		$this->output($Return);
	...
	public function delete_timelog() {
		...
		$id = $this->uri->segment(4);
		$Return['csrf_hash'] = $this->security->get_csrf_hash();
		$result = $this->Project_model->delete_record_timelog($id);
		if(isset($id)) {
