...
class Performance_variable extends MY_Controller {
    ...
    public function add_variable_kpi () {
        if($this->input->post('add_type')=='kpi_variable') {
            ...
            if($this->input->post('kpi_variable')==='') {
                $Return['error'] = $this->lang->line('umb_error_kpi_variable_field');
            }
            $variable_targeted_date = $this->input->post('variable_targeted_date');
            if (empty($variable_targeted_date)) {
                $Return['error'] = $this->lang->line('umb_error_kpi_targeted_date_field');
            }
            if($Return['error']!='') {
                $this->output($Return);
            }
            ...
            if ($result == TRUE) {
                $Return['result'] = $this->lang->line('umb_sukses_kpi_variable_ditambahkan');
            }
            $this->output($Return);
            exit;	
        }
    ...
    public function list_variable () {
        ...
        foreach($variable->result() as $r) {
            ...
            if ($r->user_id != $session['user_id']) {
                $action = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('umb_edit').'"><button type="button" class="btn btn-secondary btn-sm m-b-0-0 waves-effect waves-light" data-toggle="modal" data-target=".edit-modal-variable-data" data-variable_id="'. $r->id . '"><i class="fas fa-pencil-alt-square-o"></i></button></span>';
            } else {
                //$action = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('umb_edit').'"><button type="button" class="btn btn-secondary btn-sm m-b-0-0 waves-effect waves-light" data-toggle="modal" data-target=".edit-modal-variable-data" data-variable_id="'. $r->id . '"><i class="fas fa-pencil-alt-square-o"></i></button></span>';
                $action = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('umb_edit').'"><button type="button" class="btn btn-secondary btn-sm m-b-0-0 waves-effect waves-light" data-toggle="modal" data-target=".edit-modal-variable-data" data-variable_id="'. $r->id . '"><i class="fas fa-pencil-alt-square-o"></i></button></span><span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('umb_delete').'"><button type="button" class="btn btn-outline-danger btn-sm m-b-0-0 waves-effect waves-light delete-variable" data-toggle="modal" data-target=".delete-modal-variable" data-record-id="'. $r->id . '"><i class="fas fa-trash-restore-o"></i></button></span>';
            }
    ...
    public function delete_variable() {
        if($this->input->post('type')=='delete') {
            ...
            $result = $this->Performance_variable_model->delete_record_variable($id);
            if(isset($id)) {
                $Return['result'] = $this->lang->line('kpi_variable_dihapus_successful');
            } else {
    ...
    public function read_record_variable()
    {
        $data['title'] = $this->Umb_model->site_title();
        $variable_id = $this->input->get('variable_id');
        $result = $this->Performance_variable_model->read_informasi_variable($variable_id);
    ...
    public function edit_variable() {
        ...
        if($this->input->post('edit_type')=='variable') {
            ...
            $result = $this->Performance_variable_model->update_record_variable($data,$id);

            if ($result == TRUE) {
                $Return['result'] = $this->lang->line('umb_sukses_kpi_variable_diperbarui');
            } else {
                $Return['error'] = $this->lang->line('umb_error_msg');
            }
            $this->output($Return);
    ...
    public function list_variable_quarter () {
        $data['title'] = $this->Umb_model->site_title();
        $session = $this->session->userdata('username');
        // if(!empty($session)){ 
        //  $this->load->view("admin/performance_kpi", $data);
