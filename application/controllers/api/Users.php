    ...
    public function users_get(){   
        ...
        if($user_id === NULL){
            ...
            if($users){
                ...
                $this->response(array('status'=> false, 'message'=> 'The Specified user could not be found'), REST_Controller::HTTP_NOT_FOUND);
            }
        }
        $user_id = (int) $user_id;
        if ($user_id <= 0){
    ...
    public function users_delete(){    
        ...
        if(isset($user['user_id'])){
            ...
            if(!$deleted){
                ...
                $this->response(array('status'=> 'success', 'message'=> 'User deleted'), REST_Controller::HTTP_NO_CONTENT);
            }
        } else{
            $this->response(array('status'=> 'failure', 'message'=> 'The Specified user could not be found'), REST_Controller::HTTP_NOT_FOUND);
        }  
